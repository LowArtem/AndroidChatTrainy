...
class StartStopRemoteActions(
    ...
    suspend fun appClosed() = withContext(Dispatchers.IO) {
        ...
        if (user != null) {
            authControllerRemote.updateUser(
                id = user.id,
                user = UserFull(user.id, null, user.username, user.password, false, Calendar.getInstance().time)
            )
        }
        else {
    ...
    suspend fun appStarted() = withContext(Dispatchers.IO) {
        ...
        if (user != null) {
            authControllerRemote.updateUser(
                id = user.id,
                user = UserFull(user.id, null, user.username, user.password, true, Calendar.getInstance().time)
            )
        }
        else {
